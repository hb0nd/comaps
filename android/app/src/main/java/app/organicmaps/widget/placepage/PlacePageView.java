...
//    showTaxiOffer(mapObject);
  ...
  private void refreshOpenState()
  {
    ...
    if (timetables != null && timetables.length != 0)
    {
      ...
      if (poiState.state == OhState.RuleState.Unknown)
      {
      ...
      final boolean isOpen = poiState.state == OhState.RuleState.Open; // False == Closed due to early exit for Unknown
      final long nextStateTime = isOpen ? poiState.nextTimeClosed : poiState.nextTimeOpen;
      final int minsToNextState = (int) ((nextStateTime - (System.currentTimeMillis() / 1000)) / 60);

      final ForegroundColorSpan colorGreen = new ForegroundColorSpan(ContextCompat.getColor(requireContext(), R.color.base_green));
      final ForegroundColorSpan colorYellow = new ForegroundColorSpan(ContextCompat.getColor(requireContext(), R.color.base_yellow));
      final ForegroundColorSpan colorRed = new ForegroundColorSpan(ContextCompat.getColor(requireContext(), R.color.base_red));
      ...
      {
        //TODO: add open time
        final String fChangeMins = minsToNextState + " " + getString(R.string.minute);
        final int fChangeString = isOpen ? R.string.closes_in : R.string.opens_in;
        openStateString.append(" • "); // Add spacer
        openStateString.append(getString(fChangeString, fChangeMins), colorYellow, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
      }
